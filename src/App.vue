...
export default {
  name: 'home',
  components: { QLayout, QToolbar, QToolbarTitle, QItemSide, QSideLink, QitemMain, QRouteTab, QTabs, QBtn, QIcon, QList, QListheader, QItemMain, QSearch, Login },
  data () {
    return {
      categories: [],
      login: '',
  ...
  Created () {
    if (this.$store.state.api_key.length > 0) {
      console.log('access granted')
    }
  },

  mounted () {
    let key = this.$store.state.api_key
